...
class LoginRedirectView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            driver_profile = Driver.objects.filter(user=user, is_active=True).first()
            if driver_profile and driver_profile.assigned_bus:
                # Redirect drivers to GPS tracking page
                messages.info(request, f"Welcome {user.get_full_name()}! GPS tracking started automatically.")
                return redirect('gps_tracking:driver_tracking')
        except ImportError:
            pass

        # For non-drivers, redirect to regular dashboard
